...
var reportDir = '../reports/xml/';
var reportFile = reportDir + 'report.xml';
var testResult = [];
...
var reporterHooks = function () {
    ...
    this.registerHandler('BeforeScenario', function (event, callback) {
        ...
        var currentScenarioId =
            testResult[testResult.length - 1].id + ';' + scenario.getName().replace(/ /g, '-');
        var scenarioOutput = {
    ...
    this.registerHandler('AfterFeatures', function (event, callback) {
        var xml = junit(JSON.stringify(testResult), {
            indent: '    '
        });
        fs.open(path.join(__dirname, reportFile), 'w+', function (err, fd) {
            ...
            fs.write(fd, xml, function () {
                console.log('XML report written to: \'' + path.join(__dirname, reportFile) +
                    '\'');
                callback();
            });
        });
